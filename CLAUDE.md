...
4. **Content script** (`content.js`): finds cards with a MutationObserver and IntersectionObserver. It gets Criterion metadata (title/year/directors) from the snapshot by slug, or by fetching and parsing the film's Criterion page. Then it messages the background for the Letterboxd result and renders the badge.
...
### Catalog filters
The catalog page ships all ~3,300 rows at once, so `setupFilters` in `content.js` narrows them with a class (`.ebert-hidden`) rather than a round trip. The controls are injected into the site's own Advanced Filters panel as one more group beside Genres/Decades/Countries/Directors, and ask only what Letterboxd knows and that panel can't: minimum rating, runtime band, and the user's watched/watchlist state. Genre, decade, country and director stay the site's — these compose with them, since the site's filters navigate and ours survive the reload. The rules are pure functions in `lib/criterion.js` (`matchesFilters`, `normalizeFilters`, `inRuntimeBand`, `RUNTIME_BANDS`), which is where tests for them go.
- Nothing here touches the site's filter logic. `StoreFilters` captures its checkboxes once at init (`.filter-group-option input[type=checkbox]`) and only those feed the query string Apply navigates to, so ours are injected later and deliberately don't carry that class — Apply produces a clean `?decade=1960s`. Our options do reuse the panel's styling, which hides the checkbox and draws the dot as `label::before`. The site's Reset is hooked to clear ours too.
...
- Panel CSS is specific: `.filters .filter-group h4` sets 20px, so our section titles need `.filters .ebert-filter-group h4.ebert-section-title` to win.
