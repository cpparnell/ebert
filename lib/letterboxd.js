...
function parseFilmPage(html) {
  ...
  return {
    ...
    // Both are in the JSON-LD, so `lib/taste.js` gets its features for no extra request. The page
    // also lists them as /films/genre/ and /films/country/ links, but those repeat outside the
    // film's own panel; these two fields don't.
    genres: data.genre || [],
    countries: (data.countryOfOrigin || []).map((c) => c.name).filter(Boolean),
  };
}
...
if (typeof module !== "undefined") {
  module.exports = { slugify, squash, lbKey, candidateSlugs, titleVariants, parseFilmPage, isMatch, isSearchMatch, searchFilms, resolveFilm };
}
